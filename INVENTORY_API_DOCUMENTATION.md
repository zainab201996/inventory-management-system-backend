...
**Dashboard KPIs** - High-level inventory KPIs for dashboard widgets.

#### Get Dashboard KPIs
- **GET** `/api/dashboard`
- **Query Parameters:**
  - `threshold` (required): Non-negative number used to determine low stock (e.g. `5` means current stock < 5)
- **Response:**
  ```json
  {
    "total_stock_qty": 1234.56,
    "total_stock_value": 987654.32,
    "total_low_stock_items": 10,
    "threshold": 5
  }
  ```
- **Notes:**
  - `total_stock_qty`: Sum of current stock for all items across all stores.
  - `total_stock_value`: Sum of (current stock per item * latest rate for that item).
  - `total_low_stock_items`: Count of item-store combinations where current stock is **less than** `threshold`.
  - Soft-deleted items and stores are excluded from all KPIs.

---

### 5. Store Transfer Notes API (`/api/store-transfer-notes`)
...
- **Note:** 
  ...
  - `v_no` is **auto-generated by the backend** as a numeric string, incremented by 1 from the last voucher number; it is **locked** after creation (not updatable)
